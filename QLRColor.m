...
{
  QLRColor *color = [[QLRColor alloc] init];
  color.name = @"none";
...
{	
  ...
  if ([_colors containsObject:[name lowercaseString]]) {
    color = [QLRColor performSelector:NSSelectorFromString([NSString stringWithFormat:@"%@Color", [name lowercaseString]])];
  } else {
    color = [QLRColor defaultColor];
  }
